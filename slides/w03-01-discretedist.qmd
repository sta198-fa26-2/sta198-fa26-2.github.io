...
Consider Y chromosomes in the US population.  Roughly 51.2% of births are to babies with Y chromosomes, and 48.8% of babies do not have Y chromosomes. Thus the probability of having a baby with a Y chromosome is 0.512.
...
- Now suppose we randomly select two independent people and define a new random variable $X$ representing the number of lefties. $X$ can take the values 0, 1, or 2. Let $Y_1$ be the smoking status of the first person and $Y_2$ be the smoking status of the second person, where $Y_j$=1 if person $j$ is a lefty and takes value 0 otherwise.
...
Let $A_1$ be the event that $Y_1=1$ (person 1 is left-handed) and let $A_2$ be the event that $Y_2=1$ (person 2 is left-handed).
...
Let's use R to take a look at the distribution of the number of lefties in groups of varying size.  We can actually run an experiment in R. Let's draw a single random sample of 1000 people with probability of being left handed of 0.001. Our random variable $X$ is now the number of lefties in this big sample.
...
Suppose you conduct a survey of 1000 Chinese citizens and find that 30 are left-handed. If we estimate the probability of left-handedness based on the proportion who reported it in this survey, what is our estimate? Based on these data, do you think the rate of left-handedness in China is equivalent to that for the US as a whole? Why or why not?
...
- In the easy case in which $n=1$, $P(X \geq 1)=0.35$ and $P(X>2)=0$*
...
$P(X=1 \mid \pi=0.5, n=1) = 1 - P(X=1 \mid \pi=0.5, n=1) = 1-0.5 = 0.5$ or 1-dbinom(0,1,0.5) in the R console
...
or dbinom(0,2,0.5) in the R console
...
or 1-dbinom(0,2,0.5) in the R console
...
These exercises use the binomial distribution to model surveillance data in **rabies** and **avian influenza**, two classic zoonotic diseases where human, animal, and environmental health are directly linked. In both cases, a coordinated One Health surveillance program tracks parallel streams — animal-side testing (dogs, poultry) and human-side exposure or spillover data — feeding into a shared risk assessment and response system.
...
## Theme 1: Rabies Surveillance (Human & Animal Arms)
...
## Theme 2: Avian Influenza (Poultry & Human Spillover)
...
> During an active outbreak, 25% of birds in an affected poultry flock test positive on rapid antigen testing. A vet technician samples 16 birds from the flock.
>
