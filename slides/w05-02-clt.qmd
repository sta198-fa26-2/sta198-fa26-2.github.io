...
Note: bootstrapping notes based on Data Science in a Box!
...
1. Take a sample of size $n$ of variable $x$ and calculate its mean $\overline{x}_1=\frac{1}{n}\sum_{i=1}^n x_i$
...
When necessary conditions are met, we can use inference methods based on the 
CLT. Then the CLT tells us that $\bar{X}$ approximately has the distribution 
$N\left(\mu, \left(\sigma/\sqrt{n}\right)^2\right)$, where $\sigma$ is the standard deviation of the variable of interest. So for a Bernoulli distribution, $\mu=\pi$ and $\sigma=\sqrt{\pi(1-\pi)}$. Then
...
was unrelated to accessing dental health services in the prior 6 months in children $<12$ years old. In our sample the proportion of wealthy children receiving preventive dental services in this interval was roughly twice that of poor children, and testing whether these proportions differed by wealth provided evidence against the hypothesis that wealth and access are unrelated.
...
## Bootstrapping our class {.smaller}
...
```{r}
class <- c(
  "Yavuz", "Sophia", "Willa", "Allie",
  "Peter", "Harper", "Jeremin", "Jason",
  "Vennela", "Jack", "Kelsey", "Henry",
  "Annie", "Stephen", "Florence", "Olivia"
)
...
bootstrap_results <- data.frame(
  ...
  `Bootstrap 1` = sample(class, length(class), replace = TRUE),
  `Bootstrap 2` = sample(class, length(class), replace = TRUE),
  `Bootstrap 3` = sample(class, length(class), replace = TRUE),
  check.names = FALSE
)
```

**Notice:** Each bootstrap sample contains 16 names, but some students may
appear more than once while others may not appear at all.
...
```{r}
#| echo: false
knitr::kable(bootstrap_results, align = "cccc")
```
...
## Back to Flint: generate bootstrap means
